<template>
  <div
    class="bg-gradient-to-r from-blue-400 to-indigo-500 rounded-lg overflow-hidden shadow-xl max-w-sm"
  >
    <div class="p-4">
      <h2 class="text-lg font-semibold mb-2 text-white">Hello Traveler,</h2>
      <p class="text-sm mb-4 text-white">
        This website uses cookies to enhance user experience and to analyze
        performance and traffic on our website.
      </p>
      <p class="text-sm mb-4 text-white">Status:</p>
      <p class="text-sm mb-4 text-white">Pid_person:</p>
      <div class="flex justify-end space-x-4">
        <button
          class="duration-300 bg-black/0 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded"
        >
          Modifier
        </button>
        <button
          class="duration-300 bg-black/0 hover:bg-gray-90 text-white font-bold py-2 px-4 rounded"
        >
          Supprimer
        </button>
      </div>
    </div>
  </div>
</template>
